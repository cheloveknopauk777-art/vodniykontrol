...
const WaterTank = ({ level, className = "", size = "lg" }: WaterTankProps) => {
  ...
  const getSizeClasses = () => {
    switch (size) {
      case "sm":
        return { tank: "w-16 h-24", base: "w-14 h-2" };
      case "md":
        return { tank: "w-24 h-36", base: "w-20 h-3" };
      default:
        return { tank: "w-48 h-64", base: "w-40 h-4" };
    }
  };
  ...
  return (
    <div className={`relative ${className}`}>
      {/* Tank Container */}
      <div className={`relative ${sizeClasses.tank} mx-auto`}>
        {/* Glass Tank */}
        <div className="absolute inset-0 rounded-3xl bg-gradient-to-b from-white/60 to-white/30 dark:from-white/20 dark:to-white/10 backdrop-blur-sm border-2 border-white/40 dark:border-white/20 overflow-hidden shadow-card">
          {/* Water Fill */}
          <div
          ...
          {/* Water Level Marks - only for large size */}
          {size === "lg" && (
        ...
        {/* Tank Base */}
        <div className={`absolute -bottom-2 left-1/2 -translate-x-1/2 ${sizeClasses.base} bg-gradient-to-b from-muted to-muted/50 rounded-b-xl`} />
      </div>
